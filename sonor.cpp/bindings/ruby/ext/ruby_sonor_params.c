...
#define BOOL_PARAMS_SETTER(self, prop, value) \
  ...
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp); \
...
#define DEFINE_PARAM(param_name, nth) \
  ...
  rb_define_method(cParams, #param_name, ruby_sonor_params_get_ ## param_name, 0); \
  rb_define_method(cParams, #param_name "=", ruby_sonor_params_set_ ## param_name, 1);

#define RUBY_SONOR_PARAMS_PARAM_NAMES_COUNT 37
...
extern VALUE ruby_sonor_normalize_model_path(VALUE model_path);
extern VALUE rb_sonor_segment_s_new(VALUE context, int index);
extern const rb_data_type_t ruby_sonor_vad_params_type;

static ID param_names[RUBY_SONOR_PARAMS_PARAM_NAMES_COUNT];
...
static void
rb_sonor_callbcack_container_mark(ruby_sonor_callback_container *rwc)
{
  if (rwc == NULL) return;
...
static ruby_sonor_callback_container*
rb_sonor_callback_container_allocate() {
  ruby_sonor_callback_container *container;
  container = ALLOC(ruby_sonor_callback_container);
...
static void new_segment_callback(struct sonor_context *ctx, struct sonor_state *state, int n_new, void *user_data) {
  const ruby_sonor_callback_container *container = (ruby_sonor_callback_container *)user_data;
  ...
  const int n_segments = sonor_full_n_segments_from_state(state);
  for (int i = n_new; i > 0; i--) {
    int i_segment = n_segments - i;
    VALUE segment = rb_sonor_segment_s_new(*container->context, i_segment);
...
static void progress_callback(struct sonor_context *ctx, struct sonor_state *state, int progress_cur, void *user_data) {
  const ruby_sonor_callback_container *container = (ruby_sonor_callback_container *)user_data;
...
static bool encoder_begin_callback(struct sonor_context *ctx, struct sonor_state *state, void *user_data) {
  const ruby_sonor_callback_container *container = (ruby_sonor_callback_container *)user_data;
...
static void
check_thread_safety(ruby_sonor_params *rwp, VALUE *context, int n_processors)
{
  if (n_processors == 1) {
    return;
  }
  ...
  VALUE log_callback = rb_iv_get(mSonor, "log_callback");
  if (!NIL_P(log_callback)) {
    rb_raise(rb_eRuntimeError, "log callback not supported for parallel transcription");
  }
}

static void register_callbacks(ruby_sonor_params * rwp, VALUE * context) {
  if (!NIL_P(rwp->new_segment_callback_container->callback) || 0 != RARRAY_LEN(rwp->new_segment_callback_container->callbacks)) {
...
static void set_vad_params(ruby_sonor_params *rwp)
{
  ruby_sonor_vad_params * rwvp;
  TypedData_Get_Struct(rwp->vad_params, ruby_sonor_vad_params, &ruby_sonor_vad_params_type, rwvp);
...
*/
void
prepare_transcription(ruby_sonor_params *rwp, VALUE *context, int n_processors)
{
...
void
rb_sonor_params_mark(void *p)
{
  ruby_sonor_params *rwp = (ruby_sonor_params *)p;
  rb_sonor_callbcack_container_mark(rwp->new_segment_callback_container);
  rb_sonor_callbcack_container_mark(rwp->progress_callback_container);
  rb_sonor_callbcack_container_mark(rwp->encoder_begin_callback_container);
  rb_sonor_callbcack_container_mark(rwp->abort_callback_container);
  rb_gc_mark(rwp->vad_params);
}

void
ruby_sonor_params_free(ruby_sonor_params *rwp)
{
...
void
rb_sonor_params_free(void *p)
{
  ruby_sonor_params *rwp = (ruby_sonor_params *)p;
  // How to free user_data and callback only when not referred to by others?
  ruby_sonor_params_free(rwp);
  xfree(rwp);
}

static size_t
ruby_sonor_params_memsize(const void *p)
{
  const ruby_sonor_params *rwp = (const ruby_sonor_params *)p;

  return sizeof(ruby_sonor_params) + sizeof(rwp->params) + sizeof(rwp->vad_params);
}

const rb_data_type_t ruby_sonor_params_type = {
  "ruby_sonor_params",
  {
    rb_sonor_params_mark,
    rb_sonor_params_free,
    ruby_sonor_params_memsize,
  },
  0, 0,
  0
};
...
static VALUE
ruby_sonor_params_allocate(VALUE klass)
{
  ruby_sonor_params *rwp;
  VALUE obj = TypedData_Make_Struct(klass, ruby_sonor_params, &ruby_sonor_params_type, rwp);
  rwp->params = sonor_full_default_params(SONOR_SAMPLING_GREEDY);
  ...
  rwp->diarize = false;
  rwp->vad_params = TypedData_Wrap_Struct(cVADParams, &ruby_sonor_vad_params_type, (void *)&rwp->params.vad_params);
  rwp->new_segment_callback_container = rb_sonor_callback_container_allocate();
  rwp->progress_callback_container = rb_sonor_callback_container_allocate();
  rwp->encoder_begin_callback_container = rb_sonor_callback_container_allocate();
  rwp->abort_callback_container = rb_sonor_callback_container_allocate();
  return obj;
}
 ...
 */
static VALUE
ruby_sonor_params_set_language(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
  ruby_xfree((void *)rwp->params.language);
 ...
 */
static VALUE
ruby_sonor_params_get_language(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
  if (rwp->params.language) {
 ...
 */
static VALUE
ruby_sonor_params_set_translate(VALUE self, VALUE value)
{
 ...
 */
static VALUE
ruby_sonor_params_get_translate(VALUE self)
{
 ...
 */
static VALUE
ruby_sonor_params_set_no_context(VALUE self, VALUE value)
{
 ...
 */
static VALUE
ruby_sonor_params_get_no_context(VALUE self)
{
 ...
 */
static VALUE
ruby_sonor_params_set_single_segment(VALUE self, VALUE value)
{
 ...
 */
static VALUE
ruby_sonor_params_get_single_segment(VALUE self)
{
 ...
 */
static VALUE
ruby_sonor_params_set_print_special(VALUE self, VALUE value)
{
 ...
 */
static VALUE
ruby_sonor_params_get_print_special(VALUE self)
{
 ...
 */
static VALUE
ruby_sonor_params_set_print_progress(VALUE self, VALUE value)
{
 ...
 */
static VALUE
ruby_sonor_params_get_print_progress(VALUE self)
{
 ...
 */
static VALUE
ruby_sonor_params_set_print_realtime(VALUE self, VALUE value)
{
  BOOL_PARAMS_SETTER(self, print_realtime, value)
}
/*
 * If true, prints results from within sonor.cpp. (avoid it, use callback instead)
 *
 * call-seq:
 *   print_realtime -> bool
 */
static VALUE
ruby_sonor_params_get_print_realtime(VALUE self)
{
 ...
 */
static VALUE
ruby_sonor_params_set_print_timestamps(VALUE self, VALUE value)
{
 ...
 */
static VALUE
ruby_sonor_params_get_print_timestamps(VALUE self)
{
 ...
 */
static VALUE
ruby_sonor_params_get_carry_initial_prompt(VALUE self)
{
 ...
 */
static VALUE
ruby_sonor_params_set_carry_initial_prompt(VALUE self, VALUE value)
{
 ...
 */
static VALUE
ruby_sonor_params_set_suppress_blank(VALUE self, VALUE value)
{
 ...
 */
static VALUE
ruby_sonor_params_get_suppress_blank(VALUE self)
{
 ...
 */
static VALUE
ruby_sonor_params_set_suppress_nst(VALUE self, VALUE value)
{
 ...
 */
static VALUE
ruby_sonor_params_get_suppress_nst(VALUE self)
{
 ...
 */
static VALUE
ruby_sonor_params_get_token_timestamps(VALUE self)
{
 ...
 */
static VALUE
ruby_sonor_params_set_token_timestamps(VALUE self, VALUE value)
{
 ...
 */
static VALUE
ruby_sonor_params_get_max_len(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_max_len(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_get_split_on_word(VALUE self)
{
 ...
 */
static VALUE
ruby_sonor_params_set_split_on_word(VALUE self, VALUE value)
{
 ...
 */
static VALUE
ruby_sonor_params_get_initial_prompt(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_initial_prompt(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
  ruby_xfree((void *)rwp->params.initial_prompt);
 ...
 */
static VALUE
ruby_sonor_params_get_diarize(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
  if (rwp->diarize) {
 ...
 */
static VALUE
ruby_sonor_params_set_diarize(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
  if (value == Qfalse || value == Qnil) {
 ...
 */
static VALUE
ruby_sonor_params_get_offset(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_offset(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_get_duration(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_duration(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_get_max_text_tokens(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_max_text_tokens(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_get_temperature(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_temperature(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
...
/*
 * See https://github.com/openai/sonor/blob/f82bc59f5ea234d4b97fb2860842ed38519f7e65/sonor/decoding.py#L97
 *
 * call-seq:
 *   max_initial_ts -> Flaot
 */
static VALUE
ruby_sonor_params_get_max_initial_ts(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_max_initial_ts(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_get_length_penalty(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_length_penalty(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_get_temperature_inc(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_temperature_inc(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_get_entropy_thold(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_entropy_thold(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_get_logprob_thold(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_logprob_thold(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_get_no_speech_thold(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_no_speech_thold(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
...
static VALUE
ruby_sonor_params_get_new_segment_callback(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_new_segment_callback(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
...
static VALUE
ruby_sonor_params_get_new_segment_callback_user_data(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_new_segment_callback_user_data(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
...
static VALUE
ruby_sonor_params_get_progress_callback(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_progress_callback(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
...
static VALUE
ruby_sonor_params_get_progress_callback_user_data(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_progress_callback_user_data(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
...
static VALUE
ruby_sonor_params_get_encoder_begin_callback(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_encoder_begin_callback(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
...
static VALUE
ruby_sonor_params_get_encoder_begin_callback_user_data(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_encoder_begin_callback_user_data(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
...
static VALUE
ruby_sonor_params_get_abort_callback(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_abort_callback(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
...
static VALUE
ruby_sonor_params_get_abort_callback_user_data(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_abort_callback_user_data(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_get_vad(VALUE self)
{
  BOOL_PARAMS_GETTER(self, vad)
}

static VALUE
ruby_sonor_params_set_vad(VALUE self, VALUE value)
{
 ...
 */
static VALUE
ruby_sonor_params_set_vad_model_path(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
  ruby_xfree((void *)rwp->params.vad_model_path);
  ...
  VALUE path = ruby_sonor_normalize_model_path(value);
  rwp->params.vad_model_path = ruby_strdup(StringValueCStr(path));
  return value;
}

static VALUE
ruby_sonor_params_get_vad_model_path(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_set_vad_params(VALUE self, VALUE value)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
...
static VALUE
ruby_sonor_params_get_vad_params(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
...
#define SET_PARAM_IF_SAME(param_name) \
  if (id == id_ ## param_name) { \
    ruby_sonor_params_set_ ## param_name(self, value); \
    continue; \
  }

static VALUE
ruby_sonor_params_initialize(int argc, VALUE *argv, VALUE self)
{
  VALUE kw_hash;
  VALUE values[RUBY_SONOR_PARAMS_PARAM_NAMES_COUNT] = {Qundef};
  VALUE value;
  ruby_sonor_params *rwp;
  ID id;
  int i;
  ...
  rb_get_kwargs(kw_hash, param_names, 0, RUBY_SONOR_PARAMS_PARAM_NAMES_COUNT, values);
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);

  for (i = 0; i < RUBY_SONOR_PARAMS_PARAM_NAMES_COUNT; i++) {
    id = param_names[i];
    value = values[i];
    if (value == Qundef) {
      continue;
...
/*
 * Hook called on new segment. Yields each Sonor::Segment.
 *
 *   sonor.on_new_segment do |segment|
 *     # ...
 *   end
 *
 * call-seq:
 ...
 */
static VALUE
ruby_sonor_params_on_new_segment(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
 ...
 */
static VALUE
ruby_sonor_params_on_progress(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
  const VALUE blk = rb_block_proc();
 ...
 */
static VALUE
ruby_sonor_params_on_encoder_begin(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
  const VALUE blk = rb_block_proc();
 ...
 */
static VALUE
ruby_sonor_params_abort_on(VALUE self)
{
  ruby_sonor_params *rwp;
  TypedData_Get_Struct(self, ruby_sonor_params, &ruby_sonor_params_type, rwp);
  const VALUE blk = rb_block_proc();
...
void
init_ruby_sonor_params(VALUE *mSonor)
{
  cParams  = rb_define_class_under(*mSonor, "Params", rb_cObject);

  rb_define_alloc_func(cParams, ruby_sonor_params_allocate);
  rb_define_method(cParams, "initialize", ruby_sonor_params_initialize, -1);
  ...
  rb_define_method(cParams, "on_new_segment", ruby_sonor_params_on_new_segment, 0);
  rb_define_method(cParams, "on_progress", ruby_sonor_params_on_progress, 0);
  rb_define_method(cParams, "on_encoder_begin", ruby_sonor_params_on_encoder_begin, 0);
  rb_define_method(cParams, "abort_on", ruby_sonor_params_abort_on, 0);
}
