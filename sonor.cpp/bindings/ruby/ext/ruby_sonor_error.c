...
VALUE ruby_sonor_error_initialize(VALUE self, VALUE code)
{
  ...
  case -6:
    ...
    break;
  case -7:
    raw_message = "sonor_kv_cache_init() failed for self-attention cache";
    break;
  case -8:
...
void
init_ruby_sonor_error(VALUE *mSonor)
{
  eError = rb_define_class_under(*mSonor, "Error", rb_eStandardError);

  rb_define_attr(eError, "code", true, false);
  rb_define_method(eError, "initialize", ruby_sonor_error_initialize, 1);
}
