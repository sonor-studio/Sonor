...
static VALUE \
ruby_sonor_context_params_get_ ## name(VALUE self) { \
  ruby_sonor_context_params *rwcp; \
  GetContextParams(self, rwcp); \
  return rwcp->params.name ? Qtrue : Qfalse; \
} \
static VALUE \
ruby_sonor_context_params_set_ ## name(VALUE self, VALUE value) { \
  ruby_sonor_context_params *rwcp; \
  GetContextParams(self, rwcp); \
...
#define DEFINE_PARAM(param_name, nth) \
  ...
  rb_define_method(cContextParams, #param_name, ruby_sonor_context_params_get_ ## param_name, 0); \
  rb_define_method(cContextParams, #param_name "=", ruby_sonor_context_params_set_ ## param_name, 1);
...
static size_t
ruby_sonor_context_params_memsize(const void *p)
{
  const ruby_sonor_context_params *rwcp = (ruby_sonor_context_params *)p;
  if (!rwcp) {
    return 0;
  }
  return sizeof(ruby_sonor_context_params);
}

const rb_data_type_t ruby_sonor_context_params_type = {
  "ruby_sonor_context_params",
  {0, RUBY_DEFAULT_FREE, ruby_sonor_context_params_memsize,},
  0, 0,
  0
};

static VALUE
ruby_sonor_context_params_s_allocate(VALUE klass)
{
  ruby_sonor_context_params *rwcp;
  return TypedData_Make_Struct(klass, ruby_sonor_context_params, &ruby_sonor_context_params_type, rwcp);
}
...
static VALUE
ruby_sonor_context_params_get_dtw_n_top(VALUE self) {
  ruby_sonor_context_params *rwcp;
  GetContextParams(self, rwcp);
...
static VALUE
ruby_sonor_context_params_set_dtw_n_top(VALUE self, VALUE value) {
  ruby_sonor_context_params *rwcp;
  GetContextParams(self, rwcp);
...
#define SET_PARAM_IF_SAME(param_name) \
  if (id == id_ ## param_name) { \
    ruby_sonor_context_params_set_ ## param_name(self, value); \
    continue; \
  }

static VALUE
ruby_sonor_context_params_initialize(int argc, VALUE *argv, VALUE self)
{
  ruby_sonor_context_params *rwcp;
  TypedData_Get_Struct(self, ruby_sonor_context_params, &ruby_sonor_context_params_type, rwcp);
  rwcp->params = sonor_context_default_params();
...
void
init_ruby_sonor_context_params(VALUE *cContext)
{
  cContextParams = rb_define_class_under(*cContext, "Params", rb_cObject);

  rb_define_alloc_func(cContextParams, ruby_sonor_context_params_s_allocate);
  rb_define_method(cContextParams, "initialize", ruby_sonor_context_params_initialize, -1);
