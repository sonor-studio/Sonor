...
extern const rb_data_type_t ruby_sonor_context_params_type;
extern VALUE ruby_sonor_transcribe(int argc, VALUE *argv, VALUE self);
extern VALUE rb_sonor_model_s_new(VALUE context);
extern VALUE rb_sonor_segment_s_new(VALUE context, int index);
extern void prepare_transcription(ruby_sonor_params *rwp, VALUE *context, int n_processors);
...
static void
ruby_sonor_free(ruby_sonor *rw)
{
  if (rw->context) {
    sonor_free(rw->context);
    rw->context = NULL;
  }
}

void
rb_sonor_mark(ruby_sonor *rw)
{
  // call rb_gc_mark on any ruby references in rw
}

void
rb_sonor_free(void *p)
{
  ruby_sonor *rw = (ruby_sonor *)p;
  ruby_sonor_free(rw);
  free(rw);
}

static size_t
ruby_sonor_memsize(const void *p)
{
  const ruby_sonor *rw = (const ruby_sonor *)p;
...
const rb_data_type_t ruby_sonor_type = {
  "ruby_sonor",
  {0, rb_sonor_free, ruby_sonor_memsize,},
  0, 0,
  0
};

static VALUE
ruby_sonor_allocate(VALUE klass)
{
  ruby_sonor *rw;
  VALUE obj = TypedData_Make_Struct(klass, ruby_sonor, &ruby_sonor_type, rw);
  rw->context = NULL;
  return obj;
}

VALUE
ruby_sonor_normalize_model_path(VALUE model_path)
{
...
/*
 * call-seq:
 *   new("base.en") -> Sonor::Context
 *   new("path/to/model.bin") -> Sonor::Context
 *   new(Sonor::Model::URI.new("https://example.net/uri/of/model.bin")) -> Sonor::Context
 */
static VALUE
ruby_sonor_initialize(int argc, VALUE *argv, VALUE self)
{
  ruby_sonor *rw;
  VALUE sonor_model_file_path;
  VALUE context_params;
  struct sonor_context_params params;

  // TODO: we can support init from buffer here too maybe another ruby object to expose
  rb_scan_args(argc, argv, "11", &sonor_model_file_path, &context_params);
  TypedData_Get_Struct(self, ruby_sonor, &ruby_sonor_type, rw);

  sonor_model_file_path = ruby_sonor_normalize_model_path(sonor_model_file_path);
  if (!rb_respond_to(sonor_model_file_path, id_to_s)) {
    rb_raise(rb_eRuntimeError, "Expected file path to model to initialize Sonor::Context");
  }
  if (NIL_P(context_params)) {
    params = sonor_context_default_params();
  } else {
    ruby_sonor_context_params *rwcp;
    GetContextParams(context_params, rwcp);
    params = rwcp->params;
  }
  rw->context = sonor_init_from_file_with_params(StringValueCStr(sonor_model_file_path), params);
  if (rw->context == NULL) {
    rb_raise(rb_eRuntimeError, "error: failed to initialize sonor context");
  }
  return self;
}
 ...
 */
VALUE ruby_sonor_model_n_vocab(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
 ...
 */
VALUE ruby_sonor_model_n_audio_ctx(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
 ...
 */
VALUE ruby_sonor_model_n_audio_state(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
 ...
 */
VALUE ruby_sonor_model_n_audio_head(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
 ...
 */
VALUE ruby_sonor_model_n_audio_layer(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
 ...
 */
VALUE ruby_sonor_model_n_text_ctx(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
 ...
 */
VALUE ruby_sonor_model_n_text_state(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
 ...
 */
VALUE ruby_sonor_model_n_text_head(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
 ...
 */
VALUE ruby_sonor_model_n_text_layer(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
 ...
 */
VALUE ruby_sonor_model_n_mels(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
 ...
 *   model_ftype -> Integer
 */
VALUE ruby_sonor_model_ftype(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
 ...
 *   model_type -> String
 */
VALUE ruby_sonor_model_type(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
...
full_body(VALUE rb_args)
{
  ...
  TypedData_Get_Struct(*args->params, ruby_sonor_params, &ruby_sonor_params_type, rwp);

  prepare_transcription(rwp, args->context, 1);
  int result = sonor_full(rw->context, rwp->params, args->samples, args->n_samples);
 ...
 */
VALUE ruby_sonor_full(int argc, VALUE *argv, VALUE self)
{
  if (argc < 2 || argc > 3) {
...
full_parallel_body(VALUE rb_args)
{
  ...
  int result = sonor_full_parallel(rw->context, rwp->params, args->samples, args->n_samples, args->n_processors);
 ...
 */
static VALUE
ruby_sonor_full_parallel(int argc, VALUE *argv,VALUE self)
{
  if (argc < 2 || argc > 4) {
 ...
 */
static VALUE
ruby_sonor_full_n_segments(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
  return INT2NUM(sonor_full_n_segments(rw->context));
}

/*
 * Language ID, which can be converted to string by Sonor.lang_str and Sonor.lang_str_full.
 *
 * call-seq:
 *   full_lang_id -> Integer
 */
static VALUE
ruby_sonor_full_lang_id(VALUE self)
{
  ruby_sonor *rw;
  GetContext(self, rw);
  return INT2NUM(sonor_full_lang_id(rw->context));
}

static int ruby_sonor_full_check_segment_index(const ruby_sonor * rw, const VALUE i_segment)
{
  const int c_i_segment = NUM2INT(i_segment);
  if (c_i_segment < 0 || c_i_segment >= sonor_full_n_segments(rw->context)) {
 ...
 */
static VALUE
ruby_sonor_full_get_segment_t0(VALUE self, VALUE i_segment)
{
  ruby_sonor *rw;
  GetContext(self, rw);
  const int c_i_segment = ruby_sonor_full_check_segment_index(rw, i_segment);
  const int64_t t0 = sonor_full_get_segment_t0(rw->context, c_i_segment);
  return LONG2NUM(t0);
}
 ...
 */
static VALUE
ruby_sonor_full_get_segment_t1(VALUE self, VALUE i_segment)
{
  ruby_sonor *rw;
  GetContext(self, rw);
  const int c_i_segment = ruby_sonor_full_check_segment_index(rw, i_segment);
  const int64_t t1 = sonor_full_get_segment_t1(rw->context, c_i_segment);
  return LONG2NUM(t1);
}
 ...
 */
static VALUE
ruby_sonor_full_get_segment_speaker_turn_next(VALUE self, VALUE i_segment)
{
  ruby_sonor *rw;
  GetContext(self, rw);
  const int c_i_segment = ruby_sonor_full_check_segment_index(rw, i_segment);
  const bool speaker_turn_next = sonor_full_get_segment_speaker_turn_next(rw->context, c_i_segment);
 ...
 */
static VALUE
ruby_sonor_full_get_segment_text(VALUE self, VALUE i_segment)
{
  ruby_sonor *rw;
  GetContext(self, rw);
  const int c_i_segment = ruby_sonor_full_check_segment_index(rw, i_segment);
  const char * text = sonor_full_get_segment_text(rw->context, c_i_segment);
 ...
 */
static VALUE
ruby_sonor_full_get_segment_no_speech_prob(VALUE self, VALUE i_segment)
{
  ruby_sonor *rw;
  GetContext(self, rw);
  const int c_i_segment = ruby_sonor_full_check_segment_index(rw, i_segment);
  const float no_speech_prob = sonor_full_get_segment_no_speech_prob(rw->context, c_i_segment);
...
static VALUE
ruby_sonor_full_get_segment(VALUE self, VALUE i_segment)
{
  return rb_sonor_segment_s_new(self, NUM2INT(i_segment));
}

/*
 * Yields each Sonor::Segment:
 *
 *   sonor.transcribe("path/to/audio.wav", params)
 *   sonor.each_segment do |segment|
 *     puts segment.text
 *   end
 *
 ...
 *
 *   sonor.transcribe("path/to/audio.wav", params)
 *   enum = sonor.each_segment
 *   enum.to_a # => [#<Sonor::Segment>, ...]
 *
 * call-seq:
 ...
 */
static VALUE
ruby_sonor_each_segment(VALUE self)
{
  if (!rb_block_given_p()) {
  ...
  for (int i = 0; i < n_segments; ++i) {
    rb_yield(rb_sonor_segment_s_new(self, i));
  }
 ...
 */
static VALUE
ruby_sonor_get_model(VALUE self)
{
  return rb_sonor_model_s_new(self);
}

VALUE
init_ruby_sonor_context(VALUE *mSonor)
{
  cContext = rb_define_class_under(*mSonor, "Context", rb_cObject);

  transcribe_option_names[0] = id_n_processors;

  rb_define_alloc_func(cContext, ruby_sonor_allocate);
  rb_define_method(cContext, "initialize", ruby_sonor_initialize, -1);

  rb_define_method(cContext, "transcribe", ruby_sonor_transcribe, -1);
  rb_define_method(cContext, "model_n_vocab", ruby_sonor_model_n_vocab, 0);
  rb_define_method(cContext, "model_n_audio_ctx", ruby_sonor_model_n_audio_ctx, 0);
  rb_define_method(cContext, "model_n_audio_state", ruby_sonor_model_n_audio_state, 0);
  rb_define_method(cContext, "model_n_audio_head", ruby_sonor_model_n_audio_head, 0);
  rb_define_method(cContext, "model_n_audio_layer", ruby_sonor_model_n_audio_layer, 0);
  rb_define_method(cContext, "model_n_text_ctx", ruby_sonor_model_n_text_ctx, 0);
  rb_define_method(cContext, "model_n_text_state", ruby_sonor_model_n_text_state, 0);
  rb_define_method(cContext, "model_n_text_head", ruby_sonor_model_n_text_head, 0);
  rb_define_method(cContext, "model_n_text_layer", ruby_sonor_model_n_text_layer, 0);
  rb_define_method(cContext, "model_n_mels", ruby_sonor_model_n_mels, 0);
  rb_define_method(cContext, "model_ftype", ruby_sonor_model_ftype, 0);
  rb_define_method(cContext, "model_type", ruby_sonor_model_type, 0);
  rb_define_method(cContext, "full_n_segments", ruby_sonor_full_n_segments, 0);
  rb_define_method(cContext, "full_lang_id", ruby_sonor_full_lang_id, 0);
  rb_define_method(cContext, "full_get_segment_t0", ruby_sonor_full_get_segment_t0, 1);
  rb_define_method(cContext, "full_get_segment_t1", ruby_sonor_full_get_segment_t1, 1);
  rb_define_method(cContext, "full_get_segment_speaker_turn_next", ruby_sonor_full_get_segment_speaker_turn_next, 1);
  rb_define_method(cContext, "full_get_segment_text", ruby_sonor_full_get_segment_text, 1);
  rb_define_method(cContext, "full_get_segment_no_speech_prob", ruby_sonor_full_get_segment_no_speech_prob, 1);
  rb_define_method(cContext, "full", ruby_sonor_full, -1);
  rb_define_method(cContext, "full_parallel", ruby_sonor_full_parallel, -1);

  // High level
  rb_define_method(cContext, "full_get_segment", ruby_sonor_full_get_segment, 1);
  rb_define_method(cContext, "each_segment", ruby_sonor_each_segment, 0);

  rb_define_method(cContext, "model", ruby_sonor_get_model, 0);
