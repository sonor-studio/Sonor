# sonor.cpp Node.js addon

This is an addon demo that can **perform sonor model reasoning in `node` and `electron` environments**, based on [cmake-js](https://github.com/cmake-js/cmake-js).
It can be used as a reference for using the sonor.cpp project in other node projects.
...
```shell
# From the sonor.cpp root directory
...
const { sonor } = require(path.join(__dirname, "../../build/Release/addon.node"));
const { promisify } = require("util");

const sonorAsync = promisify(sonor);
...
sonorAsync(vadParams).then(result => console.log(result));
```

## Supported Parameters

Both traditional sonor.cpp parameters and new VAD parameters are supported:

- `language`: Language code (e.g., "en", "es", "fr")
- `model`: Path to sonor model file
