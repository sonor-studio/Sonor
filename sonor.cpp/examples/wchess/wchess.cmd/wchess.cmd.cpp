...
bool sonor_params_parse(int argc, char ** argv, sonor_params & params) {
...
int main(int argc, char ** argv) {
    ...
    if (!ctx) {
        fprintf(stderr, "%s: sonor_init_from_file_with_params() failed!\n", __func__);
        return 1;
    }

    // init audio

    if (!g_audio.init(params.capture_id, SONOR_SAMPLE_RATE)) {
