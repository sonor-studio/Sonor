...
WChess::WChess(sonor_context * ctx,
        const sonor_full_params & wparams,
        callbacks cb,
        settings s)
        : m_ctx(ctx)
        , m_wparams(wparams)
    ...
    while (get_audio(pcmf32_cur)) {
        if (!pcmf32_cur.empty()) {
            ...
            if (!have_prompt) {
                ...
                constexpr size_t MIN_SIZE = 1.2 * SONOR_SAMPLE_RATE;
...
std::string WChess::transcribe(
    ...
    if (sonor_full(m_ctx, m_wparams, pcmf32.data(), pcmf32.size()) != 0) {
        return {};
    }

    std::string result;

    const int n_segments = sonor_full_n_segments(m_ctx);
    for (int i = 0; i < n_segments; ++i) {
        const char * text = sonor_full_get_segment_text(m_ctx, i);

        result += text;

        const int n = sonor_full_n_tokens(m_ctx, i);
        for (int j = 0; j < n; ++j) {
            const auto token = sonor_full_get_token_data(m_ctx, i, j);
