...
#ifdef SONOR_FFMPEG
// as implemented in ffmpeg_trancode.cpp only embedded in common lib if sonor built with ffmpeg support
...
bool read_audio_data(const std::string & fname, std::vector<float>& pcmf32, std::vector<std::vector<float>>& pcmf32s, bool stereo) {
    ...
    decoder_config = ma_decoder_config_init(ma_format_f32, stereo ? 2 : 1, SONOR_SAMPLE_RATE);
...
int timestamp_to_sample(int64_t t, int n_samples, int sonor_sample_rate) {
    return std::max(0, std::min((int) n_samples - 1, (int) ((t*sonor_sample_rate)/100)));
}
