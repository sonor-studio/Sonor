...
void sonor_print_usage(int /*argc*/, char ** argv, const sonor_params & params, const server_params& sparams) {
    fprintf(stderr, "\n");
...
bool sonor_params_parse(int argc, char ** argv, sonor_params & params, server_params & sparams) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-h" || arg == "--help") {
            sonor_print_usage(argc, argv, params, sparams);
            exit(0);
        }
...
void sonor_print_progress_callback(struct sonor_context * /*ctx*/, struct sonor_state * /*state*/, int progress, void * user_data) {
...
std::string output_str(struct sonor_context * ctx, const sonor_params & params, std::vector<std::vector<float>> pcmf32s) {
...
void get_req_parameters(const Request & req, sonor_params & params)
{
...
int main(int argc, char ** argv) {
    ...
    std::mutex sonor_mutex;

    if (sonor_params_parse(argc, argv, params, sparams) == false) {
        sonor_print_usage(argc, argv, params, sparams);
        return 1;
    }
    ...
    if (!params.dtw.empty()) {
        ...
        if (params.dtw == "tiny") {
            cparams.dtw_aheads_preset = SONOR_AHEADS_TINY;
        }
        if (params.dtw == "tiny.en") {
            cparams.dtw_aheads_preset = SONOR_AHEADS_TINY_EN;
        }
        if (params.dtw == "base") {
            cparams.dtw_aheads_preset = SONOR_AHEADS_BASE;
        }
        if (params.dtw == "base.en") {
            cparams.dtw_aheads_preset = SONOR_AHEADS_BASE_EN;
        }
        if (params.dtw == "small") {
            cparams.dtw_aheads_preset = SONOR_AHEADS_SMALL;
        }
        if (params.dtw == "small.en") {
            cparams.dtw_aheads_preset = SONOR_AHEADS_SMALL_EN;
        }
        if (params.dtw == "medium") {
            cparams.dtw_aheads_preset = SONOR_AHEADS_MEDIUM;
        }
        if (params.dtw == "medium.en") {
            cparams.dtw_aheads_preset = SONOR_AHEADS_MEDIUM_EN;
        }
        if (params.dtw == "large.v1") {
            cparams.dtw_aheads_preset = SONOR_AHEADS_LARGE_V1;
        }
        if (params.dtw == "large.v2") {
            cparams.dtw_aheads_preset = SONOR_AHEADS_LARGE_V2;
        }
        if (params.dtw == "large.v3") {
            cparams.dtw_aheads_preset = SONOR_AHEADS_LARGE_V3;
        }
        if (params.dtw == "large.v3.turbo") { 
            cparams.dtw_aheads_preset = SONOR_AHEADS_LARGE_V3_TURBO;
        }
    ...
    svr->set_default_headers({{"Server", "sonor.cpp"},
    ...
    sonor_params default_params = params;
    ...
    svr->Post(sparams.request_path + sparams.inference_path, [&](const Request &req, Response &res){
        // acquire sonor model mutex lock
        std::lock_guard<std::mutex> lock(sonor_mutex);
        ...
        if (sparams.ffmpeg_converter) {
            ...
            const std::string temp_filename = generate_temp_filename(sparams.tmp_dir, "sonor-server", ".wav");
        ...
        // print some info about the processing
        {
            ...
            fprintf(stderr, "%s: processing '%s' (%d samples, %.1f sec), %d threads, %d processors, lang = %s, task = %s, %stimestamps = %d ...\n",
                    __func__, filename.c_str(), int(pcmf32.size()), float(pcmf32.size())/SONOR_SAMPLE_RATE,
        ...
        // run the inference
        {
            printf("Running sonor.cpp inference on %s\n", filename.c_str());
            ...
            // tell sonor to abort if the HTTP connection closed
        ...
        else if (params.response_format == srt_format)
        {
            ...
            /* try to match openai/sonor's Python format */
            std::string results = output_str(ctx, params, pcmf32s); 
            json jres = json{
                {"task", params.translate ? "translate" : "transcribe"},
                {"language", sonor_lang_str_full(sonor_full_lang_id(ctx))},
                {"duration", float(pcmf32.size())/SONOR_SAMPLE_RATE},
                {"text", results},
                {"segments", json::array()}
            };
            // Only compute language probabilities if requested (expensive operation)
            if (!params.no_language_probabilities) {
                std::vector<float> lang_probs(sonor_lang_max_id() + 1, 0.0f);
                const auto detected_lang_id = sonor_lang_auto_detect(ctx, 0, params.n_threads, lang_probs.data());
                jres["detected_language"] = sonor_lang_str_full(detected_lang_id);
                ...
                for (int i = 0; i <= sonor_lang_max_id(); ++i) {
                    if (lang_probs[i] > 0.001f) { // Only include non-negligible probabilities
                        jres["language_probabilities"][sonor_lang_str(i)] = lang_probs[i];
                    }
                }
            }
            const int n_segments = sonor_full_n_segments(ctx);
            for (int i = 0; i < n_segments; ++i)
            {
                json segment = json{
                    {"id", i},
                    {"text", sonor_full_get_segment_text(ctx, i)},
                };

                if (!params.no_timestamps) {
                    segment["start"] = sonor_full_get_segment_t0(ctx, i) * 0.01;
                    segment["end"] = sonor_full_get_segment_t1(ctx, i) * 0.01;
                }

                float total_logprob = 0;
                const int n_tokens = sonor_full_n_tokens(ctx, i);
                for (int j = 0; j < n_tokens; ++j) {
                    sonor_token_data token = sonor_full_get_token_data(ctx, i, j);
                    if (token.id >= sonor_token_eot(ctx)) {
                        continue;
                    }

                    segment["tokens"].push_back(token.id);
                    json word = json{{"word", sonor_full_get_token_text(ctx, i, j)}};
                    if (!params.no_timestamps) {
                ...
                segment["no_speech_prob"] = sonor_full_get_segment_no_speech_prob(ctx, i);
    ...
    svr->Post(sparams.request_path + "/load", [&](const Request &req, Response &res){
        ...
        // sonor init
        ctx = sonor_init_from_file_with_params(model.c_str(), cparams);
    ...
    printf("\nsonor server listening at http://%s:%d\n\n", sparams.hostname.c_str(), sparams.port);
