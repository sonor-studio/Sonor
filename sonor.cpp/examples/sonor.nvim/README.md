...
- Download the `base.en` Sonor model (140 MB):

  ```
  ./models/download-ggml-model.sh base.en
  ```

- Place the [sonor.nvim](sonor.nvim) script somewhere in your PATH and give it execute permissions:

  ```
  cp examples/sonor.nvim/sonor.nvim ~/bin/
  chmod u+x ~/bin/sonor.nvim
  ```

- Fine-tune the script to your preference and machine parameters:

  ```
  ./stream -t 8 -m models/ggml-base.en.bin --step 350 --length 10000 -f /tmp/sonor.nvim 2> /dev/null
  ```
...
- Add the following shortcuts to your `~/.config/nvim/init.vim`:
  ...
  ```
  inoremap <C-G>  <C-O>:!sonor.nvim<CR><C-O>:let @a = system("cat /tmp/sonor.nvim \| tail -n 1 \| xargs -0 \| tr -d '\\n' \| sed -e 's/^[[:space:]]*//'")<CR><C-R>a
  nnoremap <C-G>       :!sonor.nvim<CR>:let @a = system("cat /tmp/sonor.nvim \| tail -n 1 \| xargs -0 \| tr -d '\\n' \| sed -e 's/^[[:space:]]*//'")<CR>"ap
  vnoremap <C-G> c<C-O>:!sonor.nvim<CR><C-O>:let @a = system("cat /tmp/sonor.nvim \| tail -n 1 \| xargs -0 \| tr -d '\\n' \| sed -e 's/^[[:space:]]*//'")<CR><C-R>a
  ```
  
  Explanation: pressing `Ctrl-G` runs the [sonor.nvim](sonor.nvim) script which in turn calls the `stream` binary to transcribe your speech through the microphone. The results from the transcription are continuously dumped into `/tmp/sonor.nvim`. After you kill the program with `Ctrl-C`, the vim command grabs the last line from the `/tmp/sonor.nvim` file and puts it under the cursor.
...
If you find this idea interesting, you can join the discussion here: https://github.com/ggerganov/sonor.cpp/discussions/108
