# sonor.cpp/examples/sonor.swiftui

A sample SwiftUI app using [sonor.cpp](https://github.com/ggerganov/sonor.cpp/) to do voice-to-text transcriptions.
See also: [sonor.objc](https://github.com/ggerganov/sonor.cpp/tree/master/examples/sonor.objc).

### Building
 First sonor.cpp need to be built and a XCFramework needs to be created. This can be done by running
 the following script from the sonor.cpp project root:
 ```console
...
```
 ...
 Open `sonor.swiftui.xcodeproj` project in Xcode and you should be able to build and run the app on
 a simulator or a real device.

 To use the framework with a different project, the XCFramework can be added to the project by
 adding `build-apple/sonor.xcframework` by dragging and dropping it into the project navigator, or
...
2. Add the model to `sonor.swiftui.demo/Resources/models` **via Xcode**.
3. Select a sample audio file (for example, [jfk.wav](https://github.com/ggerganov/sonor.cpp/raw/master/samples/jfk.wav)).
4. Add the sample audio file to `sonor.swiftui.demo/Resources/samples` **via Xcode**.
5. Select the "Release" [^2] build configuration under "Run", then deploy and run to your device.

**Note:** Pay attention to the folder path: `sonor.swiftui.demo/Resources/models` is the appropriate directory to place resources whilst `sonor.swiftui.demo/Models` is related to actual code.
...
model.
3. Add the Core ML model (`models/ggml-base.en-encoder.mlmodelc/`) to `sonor.swiftui.demo/Resources/models` **via Xcode**.

When the example starts running you should now see that it is using the Core ML model:
```console
sonor_init_state: loading Core ML model from '/Library/Developer/CoreSimulator/Devices/25E8C27D-0253-4281-AF17-C3F2A4D1D8F4/data/Containers/Bundle/Application/3ADA7D59-7B9C-43B4-A7E1-A87183FC546A/sonor.swiftui.app/models/ggml-base.en-encoder.mlmodelc'
