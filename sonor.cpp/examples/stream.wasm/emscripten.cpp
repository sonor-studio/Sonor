...
void stream_main(size_t index, const std::string & lang) {
    ...
    bool is_multilingual = sonor_is_multilingual(g_contexts[index]);
    ...
    // 5 seconds interval
    const int64_t window_samples = 5*SONOR_SAMPLE_RATE;
    ...
    while (g_running) {
...

        {
            ...
            stream_set_status("running sonor ...");

            int ret = sonor_full(ctx, wparams, pcmf32.data(), pcmf32.size());
            if (ret != 0) {
                printf("sonor_full() failed: %d\n", ret);
                break;
            }

            const auto t_end = std::chrono::high_resolution_clock::now();

            printf("stream: sonor_full() returned %d in %f seconds\n", ret, std::chrono::duration<double>(t_end - t_start).count());
        }

        {
...

            {
                ...
                if (n_segments > 0) {
                    const char * text = sonor_full_get_segment_text(ctx, n_segments - 1);

                    const int64_t t0 = sonor_full_get_segment_t0(ctx, n_segments - 1);
                    const int64_t t1 = sonor_full_get_segment_t1(ctx, n_segments - 1);
