...
int main(int argc, char ** argv) {
    ...
    struct sonor_vad_segments * segments = sonor_vad_segments_from_probs(vctx, params);

    printf("\n");
    printf("Detected %d speech segments:\n", sonor_vad_segments_n_segments(segments));
    for (int i = 0; i < sonor_vad_segments_n_segments(segments); ++i) {
        printf("Speech segment %d: start = %.2f, end = %.2f\n", i,
               sonor_vad_segments_get_segment_t0(segments, i),
               sonor_vad_segments_get_segment_t1(segments, i));
    }
    printf("\n");

    sonor_vad_free_segments(segments);
    sonor_vad_free(vctx);
