...
struct sonor_params {
    ...
    int32_t max_len       = 0;
    int32_t best_of       = sonor_full_default_params(SONOR_SAMPLING_GREEDY).greedy.best_of;
    int32_t beam_size     = sonor_full_default_params(SONOR_SAMPLING_BEAM_SEARCH).beam_search.beam_size;
    int32_t audio_ctx     = 0;
...
static void sonor_print_usage(int argc, char ** argv, const sonor_params & params);

static char * sonor_param_turn_lowercase(char * in){
...
static bool sonor_params_parse(int argc, char ** argv, sonor_params & params) {
    if (const char * env_device = std::getenv("SONOR_ARG_DEVICE")) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (arg == "-l"    || arg == "--language")             { params.language        = sonor_param_turn_lowercase(ARGV_NEXT); }
...
static void sonor_print_progress_callback(struct sonor_context * /*ctx*/, struct sonor_state * /*state*/, int progress, void * user_data) {
    int progress_step = ((sonor_print_user_data *) user_data)->params->progress_step;
    int * progress_prev  = &(((sonor_print_user_data *) user_data)->progress_prev);
...
static void sonor_print_segment_callback(struct sonor_context * ctx, struct sonor_state * /*state*/, int n_new, void * user_data) {
    ...
    for (int i = s0; i < n_segments; i++) {
        ...
        if (params.print_colors) {
            for (int j = 0; j < sonor_full_n_tokens(ctx, i); ++j) {
                if (params.print_special == false) {
                    const sonor_token id = sonor_full_get_token_id(ctx, i, j);
                    if (id >= sonor_token_eot(ctx)) {
                        continue;
                    }
                }

                const char * text = sonor_full_get_token_text(ctx, i, j);
                const float  p    = sonor_full_get_token_p   (ctx, i, j);
        ...
        if (params.tinydiarize) {
            if (sonor_full_get_segment_speaker_turn_next(ctx, i)) {
...
static void output_txt(struct sonor_context * ctx, std::ofstream & fout, const sonor_params & params, std::vector<std::vector<float>> pcmf32s) {
    const int n_segments = sonor_full_n_segments(ctx);
...
static void output_vtt(struct sonor_context * ctx, std::ofstream & fout, const sonor_params & params, std::vector<std::vector<float>> pcmf32s) {
    fout << "WEBVTT\n\n";
...
static void output_srt(struct sonor_context * ctx, std::ofstream & fout, const sonor_params & params, std::vector<std::vector<float>> pcmf32s) {
    const int n_segments = sonor_full_n_segments(ctx);
    for (int i = 0; i < n_segments; ++i) {
        const char * text = sonor_full_get_segment_text(ctx, i);
        const int64_t t0 = sonor_full_get_segment_t0(ctx, i);
...
static void output_csv(struct sonor_context * ctx, std::ofstream & fout, const sonor_params & params, std::vector<std::vector<float>> pcmf32s) {
    const int n_segments = sonor_full_n_segments(ctx);
    fout << "start,end,";
    if (params.diarize && pcmf32s.size() == 2)
    {
    ...
    for (int i = 0; i < n_segments; ++i) {
        ...
        //need to multiply times returned from sonor_full_get_segment_t{0,1}() by 10 to get milliseconds.
        fout << 10 * t0 << "," << 10 * t1 << ",";
        if (params.diarize && pcmf32s.size() == 2)
        {
...
static void output_score(struct sonor_context * ctx, std::ofstream & fout, const sonor_params & /*params*/, std::vector<std::vector<float>> /*pcmf32s*/) {
    const int n_segments = sonor_full_n_segments(ctx);
    // fprintf(stderr,"segments: %d\n",n_segments);
    for (int i = 0; i < n_segments; ++i) {
        const int n_tokens = sonor_full_n_tokens(ctx, i);
        // fprintf(stderr,"tokens: %d\n",n_tokens);
        for (int j = 0; j < n_tokens; j++) {
            auto token = sonor_full_get_token_text(ctx, i, j);
            auto probability = sonor_full_get_token_p(ctx, i, j);
...
static void output_json(
    ...
    start_obj(nullptr);
        value_s("systeminfo", sonor_print_system_info(), false);
        start_obj("model");
            value_s("type", sonor_model_type_readable(ctx), false);
            value_b("multilingual", sonor_is_multilingual(ctx), false);
            value_i("vocab", sonor_model_n_vocab(ctx), false);
            start_obj("audio");
                value_i("ctx", sonor_model_n_audio_ctx(ctx), false);
                value_i("state", sonor_model_n_audio_state(ctx), false);
                value_i("head", sonor_model_n_audio_head(ctx), false);
                value_i("layer", sonor_model_n_audio_layer(ctx), true);
            end_obj(false);
            start_obj("text");
                value_i("ctx", sonor_model_n_text_ctx(ctx), false);
                value_i("state", sonor_model_n_text_state(ctx), false);
                value_i("head", sonor_model_n_text_head(ctx), false);
                value_i("layer", sonor_model_n_text_layer(ctx), true);
            end_obj(false);
            value_i("mels", sonor_model_n_mels(ctx), false);
            value_i("ftype", sonor_model_ftype(ctx), true);
        end_obj(false);
        start_obj("params");
        ...
        end_obj(false);
        start_obj("result");
            value_s("language", sonor_lang_str(sonor_full_lang_id(ctx)), true);
        end_obj(false);
        start_arr("transcription");
            ...
            for (int i = 0; i < n_segments; ++i) {
                ...
                start_obj(nullptr);
                    ...
                    if (full) {
                        start_arr("tokens");
                        const int n = sonor_full_n_tokens(ctx, i);
                        for (int j = 0; j < n; ++j) {
                            auto token = sonor_full_get_token_data(ctx, i, j);
                            start_obj(nullptr);
                                value_s("text", sonor_token_to_str(ctx, token.id), false);
                    ...
                    if (params.tinydiarize) {
                        value_b("speaker_turn_next", sonor_full_get_segment_speaker_turn_next(ctx, i), true);
                    }
...
static bool output_wts(struct sonor_context * ctx, std::ofstream & fout, const sonor_params & params, std::vector<std::vector<float>> pcmf32s, const char * fname_inp, float t_sec, const char * fname_out) {
    ...
    for (int i = 0; i < sonor_full_n_segments(ctx); i++) {
        const int64_t t0 = sonor_full_get_segment_t0(ctx, i);
        const int64_t t1 = sonor_full_get_segment_t1(ctx, i);

        const int n = sonor_full_n_tokens(ctx, i);

        std::vector<sonor_token_data> tokens(n);
        for (int j = 0; j < n; ++j) {
            tokens[j] = sonor_full_get_token_data(ctx, i, j);
        }
        ...
        for (int j = 0; j < n; ++j) {
...

            {
                for (int k = 0; k < n; ++k) {
                    ...
                    if (tokens[k].id >= sonor_token_eot(ctx)) {
                        continue;
                    }

                    const std::string txt = sonor_token_to_str(ctx, token2.id);
...
static void output_lrc(struct sonor_context * ctx, std::ofstream & fout, const sonor_params & params, std::vector<std::vector<float>> pcmf32s) {
    fout << "[by:sonor.cpp]\n";

    const int n_segments = sonor_full_n_segments(ctx);
    for (int i = 0; i < n_segments; ++i) {
        const char * text = sonor_full_get_segment_text(ctx, i);
        const int64_t t = sonor_full_get_segment_t0(ctx, i);
...
int main(int argc, char ** argv) {
    ...
    if (!params.dtw.empty()) {
        ...
        cparams.dtw_aheads_preset = SONOR_AHEADS_NONE;

        if (params.dtw == "tiny")      cparams.dtw_aheads_preset = SONOR_AHEADS_TINY;
        if (params.dtw == "tiny.en")   cparams.dtw_aheads_preset = SONOR_AHEADS_TINY_EN;
        if (params.dtw == "base")      cparams.dtw_aheads_preset = SONOR_AHEADS_BASE;
        if (params.dtw == "base.en")   cparams.dtw_aheads_preset = SONOR_AHEADS_BASE_EN;
        if (params.dtw == "small")     cparams.dtw_aheads_preset = SONOR_AHEADS_SMALL;
        if (params.dtw == "small.en")  cparams.dtw_aheads_preset = SONOR_AHEADS_SMALL_EN;
        if (params.dtw == "medium")    cparams.dtw_aheads_preset = SONOR_AHEADS_MEDIUM;
        if (params.dtw == "medium.en") cparams.dtw_aheads_preset = SONOR_AHEADS_MEDIUM_EN;
        if (params.dtw == "large.v1")  cparams.dtw_aheads_preset = SONOR_AHEADS_LARGE_V1;
        if (params.dtw == "large.v2")  cparams.dtw_aheads_preset = SONOR_AHEADS_LARGE_V2;
        if (params.dtw == "large.v3")  cparams.dtw_aheads_preset = SONOR_AHEADS_LARGE_V3;
        if (params.dtw == "large.v3.turbo")  cparams.dtw_aheads_preset = SONOR_AHEADS_LARGE_V3_TURBO;
    ...
    // initialize openvino encoder. this has no effect on sonor.cpp builds that don't have OpenVINO configured
    sonor_ctx_init_openvino_encoder(ctx, nullptr, params.openvino_encode_device.c_str(), nullptr);
    ...
    for (int f = 0; f < (int) params.fname_inp.size(); ++f) {
        ...
        struct fout_factory {
            ...
            bool used_stdout;
            decltype(sonor_print_segment_callback) * const print_segment_callback;
            std::ofstream fout;

            fout_factory (const std::string & fname_out_, const std::string & fname_inp, sonor_params & params) :
                    fname_out{!fname_out_.empty() ? fname_out_ : fname_inp},
                    basename_length{fname_out.size()},
                    is_stdout{fname_out == "-"},
                    used_stdout{},
                    print_segment_callback{is_stdout ? nullptr : sonor_print_segment_callback} {
        ...
        // run the inference
        {
            ...
            wparams.strategy = (params.beam_size > 1 || use_grammar) ? SONOR_SAMPLING_BEAM_SEARCH : SONOR_SAMPLING_GREEDY;
            ...
            sonor_print_user_data user_data = { &params, &pcmf32s, 0 };
            ...
            if (wparams.print_progress) {
                wparams.progress_callback           = sonor_print_progress_callback;
            ...
            // the callback is called before every encoder run - if it returns false, the processing is aborted
            {
                ...
                wparams.encoder_begin_callback = [](struct sonor_context * /*ctx*/, struct sonor_state * /*state*/, void * user_data) {
...
#define output_ext(ext, ...) output_func(output_##ext, "." #ext, params.output_##ext, __VA_ARGS__)
            ...
            output_ext(wts, pcmf32s, fname_inp.c_str(), float(pcmf32.size() + 1000)/SONOR_SAMPLE_RATE, fout_factory.fname_out.c_str());
