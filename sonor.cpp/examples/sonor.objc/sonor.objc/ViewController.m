...
{
    format->mSampleRate       = SONOR_SAMPLE_RATE;
...
#endif
        stateInp.ctx = sonor_init_from_file_with_params([modelPath UTF8String], cparams);
...
- (IBAction)onTranscribe:(id)sender {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        sonor_reset_timings(self->stateInp.ctx);

        if (sonor_full(self->stateInp.ctx, params, self->stateInp.audioBufferF32, self->stateInp.n_samples) != 0) {
        ...
        sonor_print_timings(self->stateInp.ctx);
        ...
        int n_segments = sonor_full_n_segments(self->stateInp.ctx);
        for (int i = 0; i < n_segments; i++) {
            const char * text_cur = sonor_full_get_segment_text(self->stateInp.ctx, i);
