...
static bool sonor_params_parse(int argc, char ** argv, sonor_params & params) {
    for (int i = 1; i < argc; i++) {
        ...
        else if (arg == "-mw"  || arg == "--model-sonor")  { params.model_wsp      = argv[++i]; }
...
void sonor_print_usage(int /*argc*/, char ** argv, const sonor_params & params) {
    ...
    fprintf(stderr, "  -mw FILE, --model-sonor  [%-7s] sonor model file\n",                          params.model_wsp.c_str());
...
static std::string transcribe(
    ...
    prompt_tokens.resize(sonor_tokenize(ctx, prompt_text.c_str(), prompt_tokens.data(), prompt_tokens.size()));
...
const std::string k_prompt_sonor = R"(A conversation with a person called {1}.)";
...
int main(int argc, char ** argv) {
    ...
    struct sonor_context * ctx_wsp = sonor_init_from_file_with_params(params.model_wsp.c_str(), cparams);
    if (!ctx_wsp) {
        fprintf(stderr, "No sonor.cpp model specified. Please provide using -mw <modelfile>\n");
        return 1;
    }
    ...
    // print some info about the processing
    {
        ...
        if (!sonor_is_multilingual(ctx_wsp)) {
    ...
    const std::string prompt_sonor = ::replace(k_prompt_sonor, "{1}", params.bot_name);
    ...
    while (is_running) {
...

        {
            ...
            if (::vad_simple(pcmf32_cur, SONOR_SAMPLE_RATE, 1250, params.vad_thold, params.freq_thold, params.print_energy) || force_speak) {
                ...
                if (!force_speak) {
                    all_heard = ::trim(::transcribe(ctx_wsp, params, pcmf32_cur, prompt_sonor, prob0, t_ms));
                }
