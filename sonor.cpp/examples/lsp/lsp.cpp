...
static uint64_t wait_for_vad(audio_async & audio, json jparams, const sonor_params & params, uint64_t maxlength_ms, std::vector<float> & pcmf32) {
    ...
    if(time_now - start_time < 500) {
        ...
        size_t max_offset = pcmf32.size() - SONOR_SAMPLE_RATE;
        for(size_t offset=0;offset < max_offset;offset+=SONOR_SAMPLE_RATE/10) {
            std::vector<float> audio_chunk(&pcmf32[offset], &pcmf32[offset+SONOR_SAMPLE_RATE]);
            if(::vad_simple(audio_chunk, SONOR_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold, params.print_energy)) {
                pcmf32.resize(offset+SONOR_SAMPLE_RATE);
                if (offset*1000/SONOR_SAMPLE_RATE+1000 > maxlength_ms) {
                    //remove samples from the beginning
                    pcmf32.erase(pcmf32.begin(),pcmf32.end()-(maxlength_ms*SONOR_SAMPLE_RATE/1000));
                    fprintf(stderr, "Shortened samples");
                }
                return start_time + offset*1000/SONOR_SAMPLE_RATE+1000;
            }
        }
    }
    size_t window_duration = std::max((uint64_t)1000, time_now-start_time);
    audio.get(window_duration, pcmf32);
    while (!::vad_simple(pcmf32, SONOR_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold, params.print_energy)) {
...
static json unguided_transcription(struct sonor_context * ctx, audio_async &audio, json jparams, const sonor_params &params) {
    std::vector<sonor_token> prompt_tokens;
    ...
    if (jparams.contains("prompt")) {
        ...
        int n = sonor_tokenize(ctx, prompt.c_str(), prompt_tokens.data(), 1024);
    ...
    if (sonor_full(ctx, wparams, pcmf32.data(), pcmf32.size()) != 0) {
        fprintf(stderr, "%s: ERROR: sonor_full() failed\n", __func__);
        throw json{
            {"code", -32803},
            {"message", "ERROR: sonor_full() failed"}
        };
    }
    std::string result = sonor_full_get_segment_text(ctx,0);
    return json {
...
static json guided_transcription(struct sonor_context * ctx, audio_async &audio, const sonor_params &params, json jparams, std::vector<struct commandset> commandset_list) {
    ...
    if (sonor_full(ctx, wparams, pcmf32.data(), pcmf32.size()) != 0) {
        ...
        throw json{
            {"code", -32803},
            {"message", "ERROR: sonor_full() failed"}//TODO: format string (sprintf?)
        };
    }
    ...
    // NOTE: not optimal
    {
        const auto * logits = sonor_get_logits(ctx);

        std::vector<float> probs(sonor_n_vocab(ctx), 0.0f);
...
static json register_commandset(struct sonor_context * ctx, json jparams, std::vector<struct commandset> &commandset_list) {
    // TODO: check for token collision
    struct commandset cs;

    std::string  k_prompt = " select one from the available words: ";
    std::set<sonor_token> token_set;
    sonor_token tokens[32];
    for (std::string s : jparams) {
        std::vector<sonor_token> token_vec;
        ...
        const int n = sonor_tokenize(ctx, (" " + s).c_str(), tokens, 32);
        if (n < 0) {
    ...
    int n = sonor_tokenize(ctx, k_prompt.c_str(), cs.prompt_tokens.data(), 1024);
...
static json seek(struct sonor_context * /*ctx*/, audio_async & /*audio*/, json /*params*/) {
    // sonor_state has the pertinent offsets, but there also seem to be a large
...
static json parse_job(const json &body, struct sonor_context * ctx, audio_async &audio, const sonor_params &params, std::vector<struct commandset> &commandset_list) {
...
static void process_loop(struct sonor_context * ctx, audio_async &audio, const sonor_params &params) {
...
int main(int argc, char ** argv) {
    ...
    if (sonor_lang_id(params.language.c_str()) == -1) {
    ...
    if (!audio.init(params.capture_id, SONOR_SAMPLE_RATE)) {
