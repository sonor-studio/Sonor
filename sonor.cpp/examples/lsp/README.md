...
make lsp
```
Install the plugin itself by copying or symlinking sonor.vim into ~/.vim/autoload/

In your vimrc, set the path of your sonor.cpp directory and optionally add some keybinds.

```vim
let g:sonor_dir = "~/sonor.cpp"
" Start listening for commands when Ctrl - g is pressed in normal mode
nnoremap <C-G> call sonor#requestCommands()<CR>
" Start unguided transcription when Ctrl - g is pressed in insert mode
inoremap <C-G> <Cmd>call sonor#doTranscription()<CR>
```
...
Log information is sent to a special `sonor_log` buffer and can be accessed with
```vim
:e sonor_log
```
...
`g:sonor_dir`  
A full path to the sonor.cpp repo. It can be expanded in the definition like so:
```vim
let g:sonor_dir = expand("~/sonor.cpp/")
```
(The SONOR_CPP_HOME environment variable is also checked for users of the existing sonor.nvim script)

`g:sonor_lsp_path`  
Can be used to manually set the path to the language server.
If not defined, it will be inferred from the above sonor_dir

`g:sonor_model_path`  
A full path to the model to load. If not defined, it will default to ggml-base.en.bin

`g:sonor_user_commands`  
...
```vim
let g:sonor_user_commands = {"gen": "llama#doLlamaGen"}
```
...
`unguided`  
`params.no_context` Sets the corresponding sonor `no_context` param. Defaults to true. Might provide more accurate results for consecutive unguided transcriptions if those after the first are set to false.
