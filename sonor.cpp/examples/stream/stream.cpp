...
int main(int argc, char ** argv) {
    ...
    const int n_samples_step = (1e-3*params.step_ms  )*SONOR_SAMPLE_RATE;
    const int n_samples_len  = (1e-3*params.length_ms)*SONOR_SAMPLE_RATE;
    const int n_samples_keep = (1e-3*params.keep_ms  )*SONOR_SAMPLE_RATE;
    const int n_samples_30s  = (1e-3*30000.0         )*SONOR_SAMPLE_RATE;
    ...
    // print some info about the processing
    {
        ...
        fprintf(stderr, "%s: processing %d samples (step = %.1f sec / len = %.1f sec / keep = %.1f sec), %d threads, lang = %s, task = %s, timestamps = %d ...\n",
                __func__,
                n_samples_step,
                float(n_samples_step)/SONOR_SAMPLE_RATE,
                float(n_samples_len )/SONOR_SAMPLE_RATE,
                float(n_samples_keep)/SONOR_SAMPLE_RATE,
                params.n_threads,
                params.language.c_str(),
    ...
    if (params.save_audio) {
        ...
        wavWriter.open(filename, SONOR_SAMPLE_RATE, 16, 1);
    }
    ...
    while (is_running) {
        ...
        if (!use_vad) {
            ...
            if (::vad_simple(pcmf32_new, SONOR_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold, false)) {
        ...
        // run the inference
        {
            sonor_full_params wparams = sonor_full_default_params(params.beam_size > 1 ? SONOR_SAMPLING_BEAM_SEARCH : SONOR_SAMPLING_GREEDY);
            ...
            // print result;
            {
                if (!use_vad) {
                    ...
                    const int64_t t0 = std::max(0.0, t1 - pcmf32.size()*1000.0/SONOR_SAMPLE_RATE);
            ...
            if (!use_vad && (n_iter % n_new_line) == 0) {
                ...
                if (!params.no_context) {
                    ...
                    const int n_segments = sonor_full_n_segments(ctx);
                    for (int i = 0; i < n_segments; ++i) {
                        const int token_count = sonor_full_n_tokens(ctx, i);
                        for (int j = 0; j < token_count; ++j) {
                            prompt_tokens.push_back(sonor_full_get_token_id(ctx, i, j));
                        }
                    }
                }
            }
