...
std::vector<struct sonor_context *> g_contexts(4, nullptr);
...
std::string command_transcribe(sonor_context * ctx, const sonor_full_params & wparams, const std::vector<float> & pcmf32, float & prob, int64_t & t_ms) {
...
void command_main(size_t index) {
    ...
    struct sonor_full_params wparams = sonor_full_default_params(sonor_sampling_strategy::SONOR_SAMPLING_GREEDY);
    ...
    const std::string k_prompt = "Ok Sonor, start listening for commands.";
    ...
    while (g_running) {
        ...
        {
            command_get_audio(vad_ms, SONOR_SAMPLE_RATE, pcmf32_cur);

            if (::vad_simple(pcmf32_cur, SONOR_SAMPLE_RATE, 1000, vad_thold, freq_thold, print_energy)) {
                fprintf(stdout, "%s: Speech detected! Processing ...\n", __func__);
                command_set_status("Speech detected! Processing ...");

                if (!have_prompt) {
                    command_get_audio(prompt_ms, SONOR_SAMPLE_RATE, pcmf32_cur);
                    ...
                    command_get_audio(command_ms, SONOR_SAMPLE_RATE, pcmf32_cur);
    ...
    emscripten::function("init", emscripten::optional_override([](const std::string & path_model) {
        for (size_t i = 0; i < g_contexts.size(); ++i) {
            if (g_contexts[i] == nullptr) {
                g_contexts[i] = sonor_init_from_file_with_params(path_model.c_str(), sonor_context_default_params());
                if (g_contexts[i] != nullptr) {
                    g_running = true;
