...
//
// ref: https://github.com/ggml-org/sonor.cpp/issues/171
//
...
static std::string transcribe(
    ...
    sonor_full_params wparams = sonor_full_default_params(SONOR_SAMPLING_BEAM_SEARCH);
    ...
    for (int i = 0; i < n_segments; ++i) {
        ...
        for (int j = 0; j < n; ++j) {
            const auto token = sonor_full_get_token_data(ctx, i, j);
...
static int process_command_list(struct sonor_context * ctx, audio_async &audio, const sonor_params &params, std::ofstream &fout) {
    fprintf(stderr, "\n");
    ...
    std::vector<std::vector<sonor_token>> allowed_tokens;

    for (const auto & cmd : allowed_commands) {
        sonor_token tokens[1024];
        ...
        for (int l = 0; l < (int) cmd.size(); ++l) {
            ...
            const int n = sonor_tokenize(ctx, ss.c_str(), tokens, 1024);
            if (n < 0) {
    ...
    // tokenize prompt
    std::vector<sonor_token> k_tokens;
    {
        k_tokens.resize(1024);
        const int n = sonor_tokenize(ctx, k_prompt.c_str(), k_tokens.data(), 1024);
        if (n < 0) {
    ...
    while (is_running) {
        ...
        if (::vad_simple(pcmf32_cur, SONOR_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold, params.print_energy)) {
            ...
            if (sonor_full(ctx, wparams, pcmf32_cur.data(), pcmf32_cur.size()) != 0) {
                ...
                // print the commands and the respective probabilities
                {
                    ...
                    for (const auto & cmd : probs_id) {
                        ...
                        for (int token : allowed_tokens[cmd.second]) {
                            fprintf(stdout, "'%4s' %f ", sonor_token_to_str(ctx, token), probs[token]);
                        }
                        fprintf(stdout, "\n");
                    }
                }
...
static int always_prompt_transcription(struct sonor_context * ctx, audio_async & audio, const sonor_params & params, std::ofstream & fout) {
    bool is_running = true;
...
static int process_general_transcription(struct sonor_context * ctx, audio_async & audio, const sonor_params & params, std::ofstream & fout) {
    bool is_running  = true;
    ...
    std::string k_prompt = "Ok Sonor, start listening for commands.";
...

        {
            ...
            if (::vad_simple(pcmf32_cur, SONOR_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold, params.print_energy)) {
                ...
                if (!have_prompt) {
                    ...
                    //printf("len prompt:  %.4f\n", pcmf32_prompt.size() / (float) SONOR_SAMPLE_RATE);
                    //printf("len command: %.4f\n", pcmf32_cur.size() / (float) SONOR_SAMPLE_RATE);

                    // prepend 3 second of silence
                    pcmf32_cur.insert(pcmf32_cur.begin(), 3.0f*SONOR_SAMPLE_RATE, 0.0f);
